...
from .forms import LoginForm, RegiserForm, EditUserForm
...
def login_user(request):
    ...
    if  login_form.is_valid():
        ...
        if user is not None:
            ...
            login_form.add_error('user_name','کاربری با مشخصات داده شده یافت نشد')
    context = {
...
def register(request):
    if request.user.is_authenticated:
        return redirect('/')
    register_form = RegiserForm(request.POST or None)
